...
def run_optuna(x,xn,y,model_name,n_trials, timeout):
    #load the data
    xn=xn.reshape((-1, 1))
    ...
    #Split data 
    x_train, x_val, y_train, y_val = train_test_split(x, y, train_size=0.999,shuffle = True,random_state=2)
    ...
    def create_model(trial):
        ...
        input1=tf.keras.layers.Input(shape=[5,])#input1
    ...
    def objective(trial):
        ...
        model.fit([x_train[:,0:5],x_train[:,5]],y_train,batch_size=batch_size,epochs=epochs)
        pred_test_y=model.predict([x_test[:,0:5],x_test[:,5]])
        pred_train_y=model.predict([x_train[:,0:5],x_train[:,5]])
    ...
    model.fit([x_train[:,0:5], x_train[:,5]], y_train, batch_size=batch_size, epochs=epochs)
    pred_test_y=model.predict([x_test[:,0:5],x_test[:,5]])
    pred_val_y=model.predict([x_val[:,0:5],x_val[:,5]])
    pred_train_y=model.predict([x_train[:,0:5],x_train[:,5]])
    temp1=np.exp(pred_test_y.reshape(-1)*(ymax-ymin)+ymin)
    temp2=np.exp(y_test*(ymax-ymin)+ymin)
...
path='C:\\Users\\junyun_deng\\Desktop\\data\\Z11.csv'
temp=pd.read_csv(path,header=0)
data=np.array(temp)
x=data[:,2:7]
xn=data[:,7]
y=data[:,8]
run_optuna(x,xn,y,'Z11_model',n_trials=200, timeout=1200)


path='C:\\Users\\junyun_deng\\Desktop\\data\\Z21.csv'
temp=pd.read_csv(path,header=0)
data=np.array(temp)
x=data[:,2:7]
xn=data[:,7]
y=data[:,8]
run_optuna(x,xn,y,'Z21_model',n_trials=200, timeout=1200)

path='C:\\Users\\junyun_deng\\Desktop\\data\\Z22.csv'
temp=pd.read_csv(path,header=0)
data=np.array(temp)
x=data[:,2:7]
xn=data[:,7]
y=data[:,8]
run_optuna(x,xn,y,'Z22_model',n_trials=200, timeout=1200)
