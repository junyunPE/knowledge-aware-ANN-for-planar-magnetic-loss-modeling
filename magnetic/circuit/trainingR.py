...
def run_optuna(x,xn,y,model_name,n_trials, timeout):
    ...
    #Split data 
    x_train, x_val, y_train, y_val = train_test_split(x, y, train_size=100,shuffle = True,random_state=2)
    ...
    def objective(trial):
        ...
        model.fit([x_train[:,0:6],x_train[:,6]],y_train,batch_size=batch_size,epochs=epochs)
        pred_test_y=model.predict([x_test[:,0:6],x_test[:,6]])
        pred_train_y=model.predict([x_train[:,0:6],x_train[:,6]])
    ...
    model.fit([x_train[:,0:6], x_train[:,6]], y_train, batch_size=batch_size, epochs=epochs)
    pred_test_y=model.predict([x_test[:,0:6],x_test[:,6]])
    pred_val_y=model.predict([x_val[:,0:6],x_val[:,6]])
    pred_train_y=model.predict([x_train[:,0:6],x_train[:,6]])
    temp1=np.exp(pred_test_y.reshape(-1)*(ymax-ymin)+ymin)
    temp2=np.exp(y_test*(ymax-ymin)+ymin)
...
path='C:\\Users\\junyun_deng\\Desktop\\data\\R11.csv'
temp=pd.read_csv(path,header=0)
data=np.array(temp)
x=data[:,1:7]
xn=data[:,7]
y=data[:,8]
run_optuna(x,xn,y,'R11_model',n_trials=200, timeout=1200)

'''
path='C:\\Users\\junyun_deng\\Desktop\\data\\R21.csv'
temp=pd.read_csv(path,header=0)
data=np.array(temp)
x=data[:,1:7]
xn=data[:,7]
y=data[:,8]
run_optuna(x,xn,y,'R21_model',n_trials=200, timeout=1200)

path='C:\\Users\\junyun_deng\\Desktop\\data\\R22.csv'
temp=pd.read_csv(path,header=0)
data=np.array(temp)
x=data[:,1:7]
xn=data[:,7]
y=data[:,8]
run_optuna(x,xn,y,'R22_model',n_trials=200, timeout=1200)
'''
