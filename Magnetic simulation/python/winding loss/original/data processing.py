...
data=pd.DataFrame({'airgap':x_test[:,0]*apstd+apmean,'frequency':x_test[:,1]*fstd+fmean,'thickness':x_test[:,2]*tkstd+tkmean,\
    'dis_ver':x_test[:,3]*dvstd+dvmean,'current':x_test[:,4]*Istd+Imean,'pred_test_y':temp1,'y_test':temp2,'prediction error':err_test})
data.to_csv('D:\\file\\ECCE\\Magnetic simulation\\python\\winding loss\\original\\data_test.csv')


data1=pd.DataFrame({'airgap':x_val[:,0]*apstd+apmean,'frequency':x_val[:,1]*fstd+fmean,'thickness':x_val[:,2]*tkstd+tkmean,\
    'dis_ver':x_val[:,3]*dvstd+dvmean,'current':x_val[:,4]*Istd+Imean,'pred_eval_y':temp3,'y_eval':temp4,'prediction error':err_val})
data1.to_csv('D:\\file\\ECCE\\Magnetic simulation\\python\\winding loss\\original\\data_eval.csv')
