...
path='D:\\Magnetic simulation\\python\\core loss\\programme\\parallel(original)\\data(3C94).csv'
...
data=pd.DataFrame({'fs':x_test[:,0]*fstd+fmean,'Bm':x_test[:,1]*Bstd+Bmean,'D':x_test[:,2]*Dstd+Dmean,\
    'pred_test_y':temp1,'y_test':temp2,'prediction error':err_test})
data.to_csv('D:\\Magnetic simulation\\python\\core loss\\programme\\parallel(original)\\data_test(3C94200).csv')
data1=pd.DataFrame({'fs':x_val[:,0]*fstd+fmean,'Bm':x_val[:,1]*Bstd+Bmean,'D':x_val[:,2]*Dstd+Dmean,\
    'pred_val_y':temp3,'y_val':temp4,'prediction error':err_val})
data1.to_csv('D:\\Magnetic simulation\\python\\core loss\\programme\\parallel(original)\\data_val(3C94200).csv')
